...
export default function ClientPart({ authList, profiles }: Props) {
  ...
  const [editRow, setEditRow] = useState<
    (User & { profile: ProfileRow | null }) | null
  >(null);
  ...
  const handleUpdate = async () => {
    ...
    try {
      const res = await fetch('/api/admin/updateProfile', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ uid: editRow.id, ...editForm }),
      });
      if (!res.ok) throw new Error(await res.text());
      setToast(true);
      setEditRow(null);
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/admin/deleteUser?uid=${editRow.id}`, {
        method: 'DELETE',
      });
      if (!res.ok) throw new Error(await res.text());
      setToast(true);
  ...
  const handleResetPwd = async () => {
    ...
    try {
      const res = await fetch('/api/admin/resetPassword', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ uid: editRow.id, newPassword: '123456' }),
      });
      if (!res.ok) throw new Error(await res.text());
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <button
          ...
          className="px-4 py-2 bg-[#59bd7b] text-white rounded hover:shadow-sm"
        >
          새 유저 만들기
        </button>
      </div>
          ...
          <tbody>
            {paged.map((r) => (
              <tr
                ...
                onClick={() => openEdit(r)}
              >
                <td className="border px-2 py-1 truncate max-w-[160px]">
                  {r.id}
                </td>
                <td className="border px-2 py-1">{r.email ?? '—'}</td>
                <td className="border px-2 py-1">{r.profile?.name ?? '—'}</td>
                <td className="border px-2 py-1">
                  {r.profile?.department ?? '—'}
                </td>
                <td className="border px-2 py-1">
                  {r.profile?.ismanager ? '관리자' : '일반'}
                </td>
                <td className="border px-2 py-1">
          ...
          <div className="grid gap-3 text-sm">
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) =>
                  setEditForm({ ...editForm, ismanager: e.target.checked })
                }
              />
              관리자 권한
            </label>
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={editForm.isinit}
                onChange={(e) =>
                  setEditForm({ ...editForm, isinit: e.target.checked })
                }
              />
              초기 등록 상태(<code>isinit</code>)
            </label>
          </div>
